...
client = commands.Bot(description="Bot prefix is #", command_prefix=commands.when_mentioned_or("prefix" ), pm_help = True)
...
async def on_ready():
     ...
     print('.......')
     print('created by bllue oak and bluebird')
     client.loop.create_task(status_task())

def is_owner(ctx):
     return ctx.message.author.id in ["455322915471097857","517729298355060736"]
...
async def ownerinfo(ctx):
    embed = discord.Embed(title="Information about owner", description="Main Creator: BLUE OAK", color=0x00ff00)
